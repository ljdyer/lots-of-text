...
class HMM():
    ...
    def __init__(self):
        ...
        self.Count_dic = {state: 0 for state in STATES}
        self.Pi_dic = self.Count_dic.copy()
        self.A_dic = {state: {s: 0.0 for s in STATES} for state in STATES}
        self.B_dic = {state: {} for state in STATES}
    ...
    def train(self, files):
        ...
        for file in files:
            with open(file, 'r', encoding='utf-8') as f:
                text = ''.join([c.lower() for c in f.read().strip()
                                if c.isalpha() or c == ' '])
                if not text:
                    print(f"Couldn't get text from file: {file}")
                    continue
    ...
    def viterbi(self, text, states, start_p, trans_p, emit_p):
        ...
        for t in range(1, len(text)):
            ...
            never_seen = not any([text[t] in emit_p[state].keys()
                                 for state in states])
